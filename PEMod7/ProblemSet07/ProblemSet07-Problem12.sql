...
SELECT
    ...
    IFNULL(MAX(I.INV_TOTAL), 0) AS 'Largest Invoice'
FROM LGCUSTOMER C
LEFT JOIN LGINVOICE I ON C.CUST_CODE = I.CUST_CODE
                      AND I.INV_TOTAL = (
                          SELECT MAX(I2.INV_TOTAL)
                          FROM LGINVOICE I2
                          WHERE I2.CUST_CODE = C.CUST_CODE
                      )
WHERE C.CUST_STATE = 'AL'
GROUP BY C.CUST_CODE, C.CUST_FNAME, C.CUST_LNAME, C.CUST_STREET, C.CUST_CITY, C.CUST_STATE, C.CUST_ZIP
